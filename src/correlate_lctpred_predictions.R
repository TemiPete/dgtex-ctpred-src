...
context_names_matrixX <- dimnames(matrixX)[[3]]
context_names_matrixY <- dimnames(matrixY)[[3]]

# find the common genes and ids
common_genes <- intersect(dimnames(matrixX)[[2]], dimnames(matrixY)[[2]])
common_individuals <- intersect(dimnames(matrixX)[[1]], dimnames(matrixY)[[1]])

print(glue::glue("INFO - Found {length(common_genes)} common genes"))
print(glue::glue("INFO - Found {length(common_individuals)} common individuals"))

possible_context_comparisons <- expand_grid(context_names_matrixX, context_names_matrixY) %>%
...
individual_correlations <- purrr::map2(.x = possible_context_comparisons$aGTEx_context, .y = possible_context_comparisons$dGTEx_context, function(x, y){
    calculate_pearson_correlation_of_two_matrices(matrixX[common_individuals, common_genes, x], matrixY[common_individuals, common_genes, y], matrix_names = c(x, y), merge_row_names = FALSE)
}) %>%
    do.call('rbind', .) %>%
...
gene_correlations <- purrr::map2(.x = possible_context_comparisons$aGTEx_context, .y = possible_context_comparisons$dGTEx_context, function(x, y){
    calculate_pearson_correlation_of_two_matrices(t(matrixX[common_individuals, common_genes, x]), t(matrixY[common_individuals, common_genes, y]), matrix_names = c(x, y), merge_row_names = FALSE)
}) %>%
    do.call('rbind', .) %>%
