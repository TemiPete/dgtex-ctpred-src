...
def ctPred_train(exp_matrix_p, params):
    ...
    pp = plot_prediction(ctPred_test, test_epi, test_exp, cell_type, fig_path) # a dictionary
    correlation = calculate_correlations(ctPred_test, test_epi, test_exp, cell_type)
...
def main():
    ...
    parser.add_argument('--metrics_file', type=str, help='Path to the file for metrics. should be a tsv file.')
    ...
    # If --data_dir argument is provided
        ...
        for cell_file in cell_files:
            correlations_list.append(ctPred_train(cell_file, params))

        correlations_dict = dict()
        for metric in correlations_list:
            correlations_dict.update(metric)
        context = list(correlations_dict.keys())
        corr_coef = list(correlations_dict.values())
        # to dataframe
        dt_metrics = pd.DataFrame(corr_coef, index = context).reset_index()
        dt_metrics.columns = ['context', 'corr_coef']
        dt_metrics.to_csv(args.metrics_file, sep = '\t')
...
# aa = np.array([list(range(1, 50))])
# aa= [a for a in aa for a in a]