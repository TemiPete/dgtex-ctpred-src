...
#     # Ensure predictions are detached and on the same device as test_labels
...
#     # Convert to NumPy arrays directly without explicit CPU transfer
#     predictions_np = predictions.cpu().numpy()
#     test_labels_np = test_labels.cpu().numpy()

#     plt.figure(figsize=(8, 6))
#     plt.scatter(test_labels_np, predictions_np, s=5)
...
def plot_prediction(model, test_features, test_labels, cell_type, fig_folder):
    ...
    predictions_np = predictions.cpu().numpy() # this fails
    test_labels_np = test_labels.cpu().numpy()

    predictions_lp = [a for a in predictions_np for a in a]
    # print(predictions_lp[0:5])
    # print(test_labels_np[0:5])

    # Compute Pearson correlation
    correlation, _ = pearsonr(test_labels_np, predictions_lp)

    # Fit a linear regression model to the predicted values and y_test
    ...
    plt.close() # added this code
    # plt.show()



def calculate_correlations(model, test_features, test_labels, cell_type):
